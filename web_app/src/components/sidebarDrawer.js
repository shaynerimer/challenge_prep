 ...
 import { Cog8ToothIcon, ChartPieIcon, FireIcon, BookOpenIcon, ChevronDownIcon } from '@heroicons/react/24/outline';
...
export default function SidebarDrawer() {
    ...
    <div className={`z-2 h-full transition-all duration-0 bg-base-200 shadow-lg flex flex-col ${expanded ? 'w-64' : 'w-16'}`}>
        ...
        <Link href="/app" className={`btn btn-ghost flex items-center ${expanded ? 'justify-start' : 'justify-center'} align-center w-full h-14 border-b-1 border-0 border-gray-300`}>
            ...
            {expanded && <span>Joke Creator</span>}
        </Link>

        <Link href="/jokes" className={`btn btn-ghost flex items-center ${expanded ? 'justify-start' : 'justify-center'} align-center w-full h-14 border-b-1 border-0 border-gray-300`}>
            <BookOpenIcon className="h-6 w-6" />
            {expanded && <span>My Jokes</span>}
        </Link>
